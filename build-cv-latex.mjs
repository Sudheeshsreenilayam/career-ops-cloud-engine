...
function buildProjects(entries) {
  ...
  for (const e of entries) {
    ...
    const title = e.url
      ? `\\href{${sanitizeUrl(e.url)}}{\\textbf{${escapeLatex(e.name)}} \\faExternalLink}`
      : `\\textbf{${escapeLatex(e.name)}}`;
    const bullets = Array.isArray(e.bullets) ? e.bullets.map(b => `            \\resumeItem{${escapeLatex(b)}}`).join('\n') : '';
    blocks.push(`    \\resumeProjectHeading\n      {${title}${context}}{${escapeLatex(e.dates)}}\n      \\resumeItemListStart\n${bullets}\n      \\resumeItemListEnd`);
  }
  return blocks.join('\n\n');
}
...
async function main() {
  ...
  try {
    ...
    const verifyScript = resolve(__dirname, 'verify-resume-compliance.mjs');
    execSync(`node "${verifyScript}" "${absInput}"`, { stdio: 'inherit' });
  } catch (err) {
    console.error('\n❌ Pre-flight compliance check failed. Aborting LaTeX compilation.');
    process.exit(1);
  }
  ...
  const substitutions = {
    ...
    SUBTITLE: escapeLatex(payload.subtitle || ''),
    TAGLINE: escapeLatex(payload.tagline || ''),
    SUMMARY_TEXT: escapeLatex(payload.summary_text || ''),
    CERTIFICATIONS: Array.isArray(payload.certifications) ? payload.certifications.map(c => `    \\resumeItem{${escapeLatex(c)}}`).join('\n') : (payload.certifications || ''),
  };
  ...
  template = template.replace(/\*\*(.*?)\*\*/g, '\\textbf{$1}');

  // Convert HTML bold to LaTeX bold
  template = template.replace(/&lt;strong&gt;(.*?)&lt;\/strong&gt;/g, '\\textbf{$1}');
  template = template.replace(/<strong>(.*?)<\/strong>/g, '\\textbf{$1}');
  template = template.replace(/&lt;b&gt;(.*?)&lt;\/b&gt;/g, '\\textbf{$1}');
  template = template.replace(/<b>(.*?)<\/b>/g, '\\textbf{$1}');
