...
export function collectSeenCompanyRoles(sources = {}, policy = {}, canonicalize = defaultCompanyNormalizer) {
  ...
  if (applicationsText) {
    ...
    for (const line of lines) {
      ...
      // Ignore items older than 90 days to allow scanning re-opened positions
      if (row.date) {
        try {
          const rowDate = new Date(row.date);
          if (!isNaN(rowDate.getTime()) && rowDate < ninetyDaysAgo) {
            continue;
          }
        } catch (e) {
  ...
  const tasks = targets.map(company => async () => {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        let activeTitleFilter = titleFilter;
        if (company.title_positive && Array.isArray(company.title_positive)) {
          activeTitleFilter = buildTitleFilter({
            positive: company.title_positive,
            negative: config.title_filter?.negative
          });
        }

        if (!activeTitleFilter(job.title)) {
          totalFilteredTitle++;
          continue;
        }
