...
3. **CONCISENESS (MANDATORY)**: Keep all explanations, findings, bullet points, and descriptions brief (1-2 sentences max per item). Do NOT write lengthy explanations or essays. This ensures the response fits within limits.
4. **BUG AVOIDANCE (INFINITE LOOPS)**: You are FORBIDDEN from outputting the pipe character (|) or generating markdown tables anywhere in your response. Convert all tables in the instructions (Blocks A, B, D, E, F, G) into clean, structured bulleted lists or nested sections (e.g., using bold key-value pairs and sub-bullets) to prevent the model from getting stuck in infinite token loops.
