...
export async function compileLatexFile(absPath, content, outputPath, compileOnly) {
  ...
  let engine = null;
  for (const candidate of [resolve('tectonic.exe'), 'tectonic', 'pdflatex']) {
    try {
  ...
  if (engine && engine.includes('tectonic')) {
  ...
  try {
    if (engine && engine.includes('tectonic')) {
      execFileSync(engine, ['--outdir', texDir, compilePath], {
        cwd: texDir,
        stdio: 'pipe',
        timeout: 120_000,
      });
  ...
  if (report.compiled) {
    ...
    try {
      if (targetPdf !== compiledPdf && existsSync(compiledPdf)) {
        await copyFile(compiledPdf, targetPdf);
        await rm(compiledPdf);
      }
      report.pdf = {
        path: targetPdf,
        sizeKB: parseFloat(((await stat(targetPdf)).size / 1024).toFixed(1)),
      };
    } catch (err) {
